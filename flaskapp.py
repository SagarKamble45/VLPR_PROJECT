from flask import Flask, render_template, Response,jsonify,request,session,send_file,make_response
...
from YOLO_Video import video_detection, cam_detection
...
@app.route('/delete_images_')
def delete__images_():
    delete_images()
    
    return  {"message": "python function called"}

@app.route('/download_videodata', methods=['GET'])
def download_videodata():
    video_data_to_csv()
    zip_filename=download_video_data() 
    # return response
    if os.path.exists(zip_filename):
        response = make_response(send_file(zip_filename, as_attachment=True))
        return response
  
@app.route('/download_livedata', methods=['GET'])
def download_livedata():
    livedata_to_csv()
    zip_filename=download_livecam_data() 
    # return response
    if os.path.exists(zip_filename):
        response = make_response(send_file(zip_filename, as_attachment=True))
        return response
